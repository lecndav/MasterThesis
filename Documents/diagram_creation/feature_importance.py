...
def main():
    ...
    data = data[signals].mean(axis=0).sort_values(ascending=True).to_frame()
    ...
    signals = signals.map(
        lambda x: ('%s_%s' % (x.split('_')[2], '_'.join(x.split('_')[3:]))))
    ax.set_yticklabels(signals)
    ax.grid(color='gray', linestyle='-', linewidth=0.25, alpha=0.5)
    plt.xlabel('Relatvie importance [%]')

    # plt.show()
    plt.savefig('../Thesis/images/feature_importance.png')
