...
def main():
    ...
    for file in os.listdir(hdf5_input):
        ...
        # data_diff = data.resample(
        #     '%sL' %
        #     window_size).apply(lambda x: x[-1] - x[0] if len(x) > 0 else 0)
        # new_column_names = dict()
        # for n in column_names:
        #     new_column_names[n] = '%s_diff' % n
        # data_diff.rename(columns=new_column_names, inplace=True)

        data = pd.concat(
            # [data_mean, data_max, data_median, data_min, data_std, data_diff],
            [data_mean, data_max, data_median, data_min, data_std],
            axis=1,
            sort=False)
